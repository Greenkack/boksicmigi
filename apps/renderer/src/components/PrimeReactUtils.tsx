...
type BadgeSeverity = 'success' | 'info' | 'warning' | 'danger' | 'secondary' | 'contrast' | null | undefined;
type Size = 'small' | 'normal' | 'large';

// Helper function to map custom severity to PrimeReact Badge severity
const mapSeverityToBadge = (severity: Severity): BadgeSeverity => {
  switch (severity) {
...
// Helper function to map custom severity to PrimeReact Button severity
const mapSeverityToButton = (severity: Severity): "success" | "info" | "warning" | "danger" | "secondary" | "contrast" | "help" | undefined => {
  switch (severity) {
    case 'warn': return 'warning';
    case 'error': return 'danger';
    case 'success': return 'success';
...
// Helper function to map custom severity to PrimeReact Tag severity
const mapSeverityToTag = (severity: Severity): "success" | "info" | "warning" | "danger" | "secondary" | "contrast" | null | undefined => {
  switch (severity) {
    case 'warn': return 'warning';
    case 'error': return 'danger';
    case 'success': return 'success';
...
export const ActionCard: React.FC<ActionCardProps> = ({
  ...
  const header = (
    <div className="flex items-center justify-between">
      ...
      {badge && (
        <Badge value={badge} severity={mapSeverityToBadge(severity)} />
      )}
    </div>
  );
  ...
  const footer = (
    <div className="flex justify-end">
      <Button 
        ...
        iconPos="right"
        size="small"
        severity={mapSeverityToButton(severity)}
        disabled={disabled}
        onClick={onAction}
      />
    </div>
    ...
    <div className="flex space-x-2">
      {actions.map((action, index) => (
        <Button
          ...
          icon={action.icon}
          size="small"
          severity={mapSeverityToButton(action.severity) || "secondary"}
        ...
        <div key={index} className="flex items-start space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              ...
              {feature.badge && (
                <Tag value={feature.badge} severity={mapSeverityToTag(feature.severity)} />
              )}
            </div>
            {feature.description && (
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  ...
                  {notification.severity && (
                    <Tag 
                      severity={mapSeverityToTag(notification.severity)} 
