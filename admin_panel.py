        ...
        with st.form(key=form_key_manual_prod_ui, clear_on_submit=False): 
            ...
            if current_product_image_b64_form and not uploaded_product_image_manual_file_form and st.session_state.product_to_edit_id_manual:
                try:
                    ...
                    st.caption(f"📊 Aktuelle Bildgröße: {len(current_product_image_b64_form)} Zeichen (Base64)")
                except Exception:
                    st.warning("⚠️ Aktuelles Bild konnte nicht angezeigt werden.")
            elif not current_product_image_b64_form and st.session_state.product_to_edit_id_manual:
                st.info("ℹ️ Kein Produktbild vorhanden. Laden Sie ein neues Bild hoch.")
            
            # Zeige Upload-Vorschau
            if uploaded_product_image_manual_file_form:
                st.image(uploaded_product_image_manual_file_form, caption=f"🆕 Neues Bild: {uploaded_product_image_manual_file_form.name} ({uploaded_product_image_manual_file_form.size} Bytes)", width=100) 
            ...
            if current_datasheet_link:
                ...
                st.info("ℹ️ Kein Datenblatt vorhanden. Laden Sie ein PDF-Datenblatt hoch.")
            
            # Zeige Upload-Vorschau
            if uploaded_datasheet_pdf_file_form:
                st.caption(f"🆕 Neues Datenblatt: {uploaded_datasheet_pdf_file_form.name} ({uploaded_datasheet_pdf_file_form.size} Bytes)")
        ...
        if submitted_manual_product_form_btn:
            if not (p_model_name_form or "").strip() or not p_category_form:
                ...
                product_data_to_save_db = {
                    ...
                    "image_base64": current_product_image_b64_form or "", # Start with current, update if new one uploaded
                ...
                if uploaded_product_image_manual_file_form:
                    if uploaded_product_image_manual_file_form.size <= 2*1024*1024: 
                        new_image_b64 = base64.b64encode(uploaded_product_image_manual_file_form.getvalue()).decode('utf-8')
                        product_data_to_save_db["image_base64"] = new_image_b64
                        st.success(f"✅ Bild erfolgreich verarbeitet ({uploaded_product_image_manual_file_form.size} Bytes)")
                    else: 
                ...
                if uploaded_datasheet_pdf_file_form:
                    if uploaded_datasheet_pdf_file_form.size <= 5*1024*1024: 
                        ...
                        st.success(f"✅ Datenblatt erfolgreich verarbeitet: {uploaded_datasheet_pdf_file_form.name} ({uploaded_datasheet_pdf_file_form.size} Bytes)")
                    else: 
                ...
                if st.session_state.product_to_edit_id_manual: 
                    ...
                    if update_product_func(product_id_for_files, product_data_to_save_db): 
                        ...
                        if uploaded_product_image_manual_file_form:
                            st.info("🖼️ Produktbild wurde erfolgreich gespeichert.")
...
def render_general_settings_extended(load_admin_setting_func: Callable, save_admin_setting_func: Callable):
    ...
    with st.form(f"global_constants_form{WIDGET_KEY_SUFFIX}"): 
        ...
        st.markdown("---"); st.subheader(get_text_local("admin_yield_settings_header", "Ertragseinstellungen für Photovoltaik-Anlage"))
        col_yield1, col_yield2 = st.columns(2)
        with col_yield1: new_global_yield_adj = st.number_input(label=get_text_local("global_yield_adjustment_percent", "Globale Ertragsanpassung (%)"), value=float(current_global_constants.get('global_yield_adjustment_percent', 0.0)), key=f"gc_yield_adj{WIDGET_KEY_SUFFIX}", format="%.2f", help="Ein positiver Wert erhöht...")
    ...
    with st.form(f"amort_cheat_form{WIDGET_KEY_SUFFIX}"):
        ...
        with col_c1:
            value_years = st.number_input("Feste Jahre / Abzug (Jahre)", value=float(cheat_current.get("value_years") or 0.0), min_value=0.0, step=0.1, format="%.2f")
        with col_c2:
...
def manage_tariff_list_ui(tariff_list_input: List[Dict[str, Any]], form_element_key_suffix: str) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    for i, tariff_entry_db_like in enumerate(actual_tariff_list_for_ui):
        ...
        ct_per_kwh_ui_val = cols_tariff_ui[2].number_input(label=f"ct/kWh #{i+1}", min_value=0.0, value=ct_per_kwh_current_value, step=0.1, key=f"{key_base}_ct", format="%.2f")
